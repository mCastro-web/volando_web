<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>

<script src="https://cdn.jsdelivr.net/npm/notyf/notyf.min.js"></script>
<script src="${pageContext.request.contextPath}/js/flyonui.js"></script>
<script type="module" src="${pageContext.request.contextPath}/js/index.js"></script>

<footer class="bg-[#0C4DC3] text-white py-6 px-10 mt-auto w-full">
  <div class="flex flex-row items-center justify-between gap-6">
    <!-- Logos UTEC - Izquierda -->
    <div class="text-left space-y-2 md:space-y-1">
        <div class="flex items-center gap-3">
            <img src="${pageContext.request.contextPath}/assets/img/logo_utec.png" alt="Logo UTEC" class="h-10 w-auto" />
            <img src="${pageContext.request.contextPath}/assets/img/logo_tecnologo.png" alt="Logo Tecnologo" class="h-10 w-auto"/>
        </div>
        <p class="mt-4 text-sm opacity-90 leading-tight hidden md:block">
            Proyecto academico desarrollado para la materia
            <strong>Programacion de Aplicaciones</strong>
        </p>
        <p class="text-sm opacity-90 leading-tight hidden md:block">
            UTEC – Universidad Tecnologica del Uruguay · Sede San Jose de Mayo
        </p>
    </div>
    
    <!-- Derecha -->
    <div class="flex flex-col md:items-end items-center gap-2 text-right md:pr-6">
        <!-- Logo VolandoUy - Solo desktop -->
        <div class="hidden md:flex items-center justify-end gap-2">
            <img src="${pageContext.request.contextPath}/favicon.png" alt="Logo VolandoUy" class="w-8 h-8" />
            <h3 class="font-semibold text-lg">VolandoUy</h3>
        </div>
        
        <!-- Texto Copyright - Visible en móvil y desktop -->
        <div class="text-xs opacity-80">
            <p>© 2025 VolandoUy</p>
            <p>UTEC · San Jose</p>
        </div>
    </div>
  </div>
</footer>