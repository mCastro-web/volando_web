...
<main class="flex flex-col items-center min-h-screen bg-base-200 text-base-content px-6 py-12">
    ...
    <div class="max-w-4xl w-full mx-auto gap-4">
        <form method="get" class="grid gap-6" action="${pageContext.request.contextPath}/CompraPaqueteServlet">
            <div>
                <label class="label">
                ...
                <select name="paquete_seleccionado" class="bg-base-200 select select-bordered w-full" onchange="this.form.submit()">
                    ...
                    <%
                        ...
                        if (paquetes != null && !paquetes.isEmpty()) {
                            String seleccionadoParam = request.getParameter("paquete_seleccionado");

                            for (String nombrePaquete : paquetes) {
                    %>
                    <option value="<%= nombrePaquete %>"
                            <%= (seleccionadoParam != null && seleccionadoParam.equals(nombrePaquete)) ? "selected" : "" %>>
                        <%= nombrePaquete %>
                    </option>
                    <%
                        }
        ...
        <% } %>
        <!-- FORM POST PARA COMPRAR -->
        <form method="post" action="${pageContext.request.contextPath}/CompraPaqueteServlet">

            <input type="hidden" name="paquete_seleccionado"
                   value="<%= request.getParameter("paquete_seleccionado") != null ? request.getParameter("paquete_seleccionado") : "" %>" />
